...
* This will replace automatically every <input> element that has the "checkbox" class.
*/


// class name we will convert to special checkbox
var _checkbox_class = "checkbox";

// create a checkbox from a given element.
...
/**
* This script add the dragging functionality to all elements with "draggable" class.
*/
...
var _draggable_class = "draggable";

// set element as draggable
// note: this also add the "draggable" css class to the element.
...
// init all draggable elements (objects with "draggable" class)
RPGUI.on_load(function()
{
 ...
 * This will replace automatically every <div> element that has the "progress" class.
 */


// class name we will convert to special progress
var _progress_class = "progress";

// create a progress from a given element.
...
RPGUI.__set_funcs["progress"] = function(elem, value)
{
	...
	var track = RPGUI.get_child_with_class(elem, "progress-track");
	var progress = RPGUI.get_child_with_class(track, "progress-fill");

	// get the two edges
	var edge_left = RPGUI.get_child_with_class(elem, "progress-left-edge");
	var edge_right = RPGUI.get_child_with_class(elem, "progress-right-edge");
...
* This will replace automatically every <input> element that has the "radio" class.
*/


// class name we will convert to special radio
var _radio_class = "radio";

// create a radio from a given element.
...
* This will replace automatically every <select> element that has the "dropdown" class.
*/


// class name we will convert to dropdown
var _dropdown_class = "dropdown";

// create a dropdown from a given element.
...
function create_dropdown(elem)
{
	...
	if (elem.id) {select_header.id = elem.id + "-dropdown-head"};
	RPGUI.add_class(select_header, "dropdown-imp dropdown-imp-header");
	RPGUI.insert_after(select_header, elem);

	// create the list to hold all the options
	var list = RPGUI.create_element("ul");
	if (elem.id) {list.id = elem.id + "-dropdown"};
	RPGUI.add_class(list, "dropdown-imp");
...
* This will replace automatically every <select> element that has the "list" class.
*/


// class name we will convert to list
var _list_class = "list";

// create a list from a given element.
// note: element must be <select> with <option> tags that will turn into the items
...
function create_list(elem)
{
	...
	// create the list to hold all the options
	var list = RPGUI.create_element("ul");
	if (elem.id) {list.id = elem.id + "-list"};
	RPGUI.add_class(list, "list-imp");
	...
	(function(elem, all_items)
	{
		...
		function _on_change(elem)
		{
			for (var i = 0; i < all_items.length; ++i)
			{
				...
				{
					RPGUI.add_class(item, "selected");
				}
				else
				{
					RPGUI.remove_class(item, "selected");
				}
			}
		}
...
* This will replace automatically every <input> element that has the "slider" class.
*/


// class name we will convert to special slider
var _slider_class = "slider";

// create a slider from a given element.
// note: element must be <input> of type "range" for this to work properly.
...
function create_slider(elem)
{
	...
	if (elem.id) {slider_container.id = elem.id + "-slider"};
	RPGUI.copy_css(elem, slider_container);
	RPGUI.add_class(slider_container, "slider-container" + golden);
	...
	// track
	var track = RPGUI.create_element("div");
	RPGUI.add_class(track, "slider-track" + golden);
	slider_container.appendChild(track);

	// left edge
	var left_edge = RPGUI.create_element("div");
	RPGUI.add_class(left_edge, "slider-left-edge" + golden);
	slider_container.appendChild(left_edge);

	// right edge
	var right_edge = RPGUI.create_element("div");
	RPGUI.add_class(right_edge, "slider-right-edge" + golden);
	slider_container.appendChild(right_edge);

	// thumb (slider value show)
	var thumb = RPGUI.create_element("div");
	RPGUI.add_class(thumb, "slider-thumb" + golden);
...
{
    RPGUI.add_class(element, "cursor-" + cursor);
};
